...
function module:SendAsync(name, data, subChannel)
	...
	assert(Utility.Cache.SubChannelsManager[subChannel] or (typeof(subChannel) == "table" and subChannel.ClassName == "SubChannelsManager") or typeof(subChannel)=="nil" or typeof(subChannel)=="string",
		('"subChannel" expected "SubChannelManager" or "string" or "nil", got %s'):format(
			typeof(subChannel)
		)
	)
	...
	if subChannel==nil then
		...
		--  Packet's m property:
		--      name/globaL/uid/max/min ie name//order or /global/order
		if name then
			packet.Data.m = name .. "/"
		else
			packet.Data.m = "/true"
...
function module:Listen(name, getComplete, subChannel, callback)
	...
	assert(typeof(subChannel)=="string" or typeof(subChannel)=="nil", string.format('"subChannel" expected "string" or "nil", got %s.', typeof(subChannel)))
	...
	while #buffer > 0 do
		...
		if packet:GetSize()>=800 then
			...
			for i=1, totalSegments do
				...
				newPacket.Data.m = (newPacket.Data.m or "/") .. ("/%s/%s/%s"):format(UID, i, totalSegments)
