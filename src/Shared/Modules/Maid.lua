...
-- @classmod Maid
-- @see Signal
-- Credit: https://github.com/Quenty/NevermoreEngine/blob/a8a2d2c1ffcf6288ec8d66f65cea593061ba2cf0/Modules/Shared/Events/Maid.lua
...
function Maid:DoCleaning()
	...
	for index, task in pairs(tasks) do
		if typeof(task) == "RBXScriptConnection" or (typeof(task)=="table" and task.Disconnect) then
			tasks[index] = nil
			task:Disconnect()
		end
	end
	...
	while task ~= nil do
		...
		if type(task) == "function" then
			task()
		elseif typeof(task) == "RBXScriptConnection" or (typeof(task)=="table" and task.Disconnect) then
			task:Disconnect()
